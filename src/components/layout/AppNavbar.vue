<template>
  <header class="max-w-[1240px] mx-auto fixed top-5 inset-x-0 z-50">
    <nav class="h-14 rounded-30 bg-white/45 backdrop-blur-md mx-5 p-2.5 border border-white/50 shadow-md">
      <div class="flex flex-row justify-between h-full">
        <div class="flex flex-row items-center gap-[16px]">
          ...
          <div class="hidden md:block">
            <ul class="flex flex-row">
              <li>
                <RouterLink to="/hotels" class="text-black font-medium px-4 py-2 hover:text-primary">住宿</RouterLink>
              </li>
              <li>
                <RouterLink to="/tickets" class="text-black font-medium px-4 py-2 hover:text-primary">體驗</RouterLink>
              </li>
              <li>
                <RouterLink to="/hotels/compare" class="text-black font-medium px-4 py-2 hover:text-primary">比較
                </RouterLink>
              </li>
              <li>
                <RouterLink to="/travel-dna/intro" class="text-black font-medium px-4 py-2 hover:text-primary">旅遊DNA
                </RouterLink>
              </li>
            </ul>
          </div>
        ...
        <ul class="flex flex-row items-center">
          <li>
            <RouterLink to="/support" class="hidden md:block text-black font-medium px-4 py-2 hover:text-primary">常見問題
            </RouterLink>
          </li>

          <li>
            <button v-if="!auth.isLoggedIn" type="button" name="login-register"
              class="text-white rounded-30 bg-primary px-4 py-2 border border-white/15 hover:bg-[#455A71]"
              @click="openLogin">
              登入/註冊
            </button>
            ...
            <div v-else class="relative hidden md:block" data-user-menu>
              <button type="button" name="user-menu"
                ...
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                  class="h-4 w-4 transition-transform duration-200" :class="isUserMenuOpen ? 'rotate-180' : ''"
                  aria-hidden="true">
              ...
              <div v-show="isUserMenuOpen"
                class="absolute right-0 mt-2 w-40 rounded-10 bg-white/80 backdrop-blur-md border border-white/50 shadow-lg overflow-hidden">
                <button type="button" name="profile"
                  class="w-full text-left px-4 py-3 text-sm text-black hover:bg-primary/10" @click="goProfileFromMenu">
                  會員中心
                </button>

                <button type="button" name="logout"
                  class="w-full text-left px-4 py-3 text-sm text-red-500 hover:bg-primary/10" @click="handleLogout">
                  登出
                </button>
              </div>
            </div>
      ...
      <template v-for="item in navItems" :key="item.name">
        <RouterLink v-if="item.path" :to="item.path" custom v-slot="{ navigate, isActive }">
          <li @click="navigate" :class="[
            'flex-1 flex flex-col gap-1 items-center justify-center py-2 rounded-full transition-all duration-200 cursor-pointer',
            isActive
  ...
  <div class="relative z-[100]">
    ...
    <Transition enter-active-class="transition-transform ease-in-out duration-300" enter-from-class="translate-x-full"
      ...
      <div v-if="isOpen"
        class="fixed inset-y-0 right-0 w-[280px] bg-white/45 backdrop-blur-md border border-white/50 m-5 rounded-30 p-8 flex flex-col gap-6">
        ...
        <nav class="flex flex-col gap-4">
          <button v-if="!auth.isLoggedIn" name="login-register" type="button"
            class="text-left text-black font-medium border-b border-primary/25 pb-2" @click="openLogin">
            登入/註冊
          </button>

          <div v-else class="flex items-center justify-between gap-3">
            <button type="button" name="profile" class="text-black font-medium hover:text-primary"
              @click="(goProfile(), (isOpen = false))">
              {{ auth.user?.user_metadata?.full_name || auth.user?.email || '會員' }}
            </button>

            <button type="button" name="logout" class="text-left text-black font-medium border-b border-primary/25 pb-2"
              @click="handleLogout">
              登出
            </button>
          </div>

          <RouterLink to="/support" class="text-black font-medium border-b border-primary/25 pb-2"
            @click="isOpen = false">
            常見問題
          </RouterLink>
          <RouterLink to="/about-us" class="text-black font-medium border-b border-primary/25 pb-2"
            @click="isOpen = false">
            關於Wantrip
          </RouterLink>
          <RouterLink to="/tos" class="text-black font-medium border-b border-primary/25 pb-2" @click="isOpen = false">
            服務條款
          </RouterLink>
          <RouterLink to="/privacy" class="text-black font-medium border-b border-primary/25 pb-2"
            @click="isOpen = false">
            隱私權聲明
          </RouterLink>
          <RouterLink to="/profile" class="text-black font-medium border-b border-primary/25 pb-2"
            @click="isOpen = false">
            會員中心
          </RouterLink>
          <RouterLink to="/travel-dna/intro" class="text-black font-medium border-b border-primary/25 pb-2"
            @click="isOpen = false">
            旅遊DNA
          </RouterLink>
        </nav>
  ...
  <LoginModal v-model="isLoginModalOpen" @signup="handleSignup" @forgot-password="handleForgotPassword"
    @social="handleSocial" />
  <RegisterModal v-model="isRegisterModalOpen" @go-login="openLoginFromRegister" @registered="closeAllAuthModal" />
</template>
...
const navItems = [
  ...
  },
  {
    ...
    path: '/hotels/compare',
    viewBox: '0 0 24 24',
    icon: 'M10.6123 21.3089C10.082 21.9452 9.13641 22.0314 8.5 21.5013L2.72368 16.5388C1.28657 15.3413 2.13336 13.0023 4.00399 13.0023H20.5C21.3284 13.0023 22 13.6739 22 14.5023C22 15.3307 21.3284 16.0023 20.5 16.0023H6.76566L10.4199 19.1966C11.0563 19.7269 11.1425 20.6725 10.6123 21.3089Z M13.3877 2.69111C13.918 2.05475 14.8636 1.96855 15.5 2.49872L21.2763 7.46119C22.7134 8.65869 21.8666 10.9977 19.996 10.9977H3.5C2.67157 10.9977 2 10.3261 2 9.4977C2 8.66927 2.67157 7.99769 3.5 7.99769H17.2343L13.5801 4.80343C12.9437 4.27313 12.8575 3.32753 13.3877 2.69111Z',
  },
  {
    name: '更多',
    path: null,
