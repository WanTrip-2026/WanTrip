<!-- <template>
  <main class="max-w-[1200px] mx-auto py-5 mt-24 mb-10">
    <div class="grid grid-cols-2 gap-5 mx-5 lg:mx-0">
      ...
      <template>
        <div v-for="hotel in visibleHotels" :key="hotel.id" class="flex flex-col space-y-2">
          <button
            type="button"
            @click="openPicker"
          ...
          <div
            ...
            class="bg-main_100 w-fit p-5 rounded-[20px] mx-auto text-center text-dark_700 shadow-sm"
          >
            目前沒有加入任何飯店可以比較。請先回飯店列表按「加入比較」。
            <div class="mt-4">
              <button class="px-5 py-2 rounded-[20px] bg-primary text-white" @click="goBackToList">
                前往飯店列表
              </button>
            </div>
          </div>
          ...
          <RouterLink :to="`/hotels/${hotel.id}`">
            class="w-full bg-primary hover:bg-main transition-all py-3 rounded-[20px] text-white
            text-center font-bold shadow-sm" > 立即預訂
          </RouterLink>
        </div>
        <div v-if="!canCompare" class="col-span-2 p-5 m-5 text-center text-dark_700">
...
<template>
  <main class="max-w-[1200px] mx-auto py-5 mt-24 mb-10">
    ...
    <button
      ...
      type="button"
      @click="openPicker"
      class="w-fit mx-auto px-6 py-3 rounded-[20px] text-center text-white text-lg font-bold shadow-sm bg-main hover:bg-main_800 transition-all"
    >
      選擇住宿（已加入 {{ compareStore.hotels.length }}/5）
    </button>
    ...
    <!-- 有資料才顯示卡片 -->
    <div v-else class="grid grid-cols-2 gap-5 mx-5 lg:mx-0 mt-5">
      <div v-for="hotel in visibleHotels" :key="hotel.id" class="flex flex-col space-y-2">
        <div
          class="w-full aspect-[16/9] bg-main_100 rounded-[20px] overflow-hidden border border-gray-300 shadow-sm"
        >
        ...
        <button
          class="w-full py-3 rounded-[20px] bg-red-500 text-white font-bold"
      ...
      >
        ...
        <div v-else class="space-y-3 max-h-[420px] overflow-auto">
          ...
          >
            ...
            <button class="text-sm text-red-500" @click="removeFromCompare(h.id)">移除</button>
          </div>
        </div>

        <div class="flex justify-end gap-3 mt-5">
          <button class="px-4 py-2 rounded-xl border" @click="clearAll">清空</button>
          <button class="px-4 py-2 rounded-xl bg-primary text-white" @click="showPicker = false">
            完成
          </button>
        </div>
      </div>
