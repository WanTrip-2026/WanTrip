    ...
    <div class="mx-5">
      ...
      <div v-else class="grid grid-cols-2 lg:grid-cols-3 gap-2">
        <div v-for="hotel in limitedHotels" :key="hotel.id" class="flex flex-col space-y-2">
          ...
          <div class="w-full aspect-[16/9] bg-main/5 rounded-20 overflow-hidden border border-gray-300 shadow-sm">
          ...
          <RouterLink :to="`/hotels/${hotel.id}`"
            class="w-full bg-primary hover:bg-main transition-all py-3 rounded-20 text-white text-center font-bold shadow-sm">
            立即預訂
          </RouterLink>
        </div>
      ...
      <div v-if="showPicker" class="fixed inset-0 z-50">
        ...
        <div
          ...
          <div v-else class="space-y-3 max-h-[420px] overflow-auto">
            <div v-for="h in compareStore.hotels" :key="h.id" class="flex items-center gap-3 border rounded-xl p-3">
          ...
          <div class="flex justify-end gap-3 mt-5">
            ...
            <button name="close-picker" class="px-4 py-2 rounded-xl bg-primary hover:bg-main text-white"
              @click="showPicker = false">
              完成
            </button>
          </div>
...
import { useCompareStore, type CompareHotel } from '@/stores/compareStore'

const router = useRouter()
const compareStore = useCompareStore()

defineProps<{ hotel?: CompareHotel }>()
...
const getDisplayFacilities = (hotel: CompareHotel) => {
